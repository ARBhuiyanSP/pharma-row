...
<div class="card mb-3">
    <div class="card-header">
		...
		<?php } ?>
        <button class="btn btn-sm btn-success"><i class="fas fa-calendar"></i> Date To Date Consumption Report </button>
		<button class="btn btn-sm btn-info" onclick="window.location.href='consumption_employee_report.php';"><i class="fas fa-users"></i> Employee Wise Consumption Report</button>
	</div>
                                ...
                                <div class="form-group">
									...
									<select class="form-control" id="material_id" name="material_id" required>
										...
										if (isset($warehouse) && !empty($warehouse)) {
											foreach ($warehouse as $data) {
												?>
												<option value="<?php  echo $data['material_id_code'] ?>"><?php echo $data['material_description'] ?></option>
											<?php }
										} ?>
									</select>
								</div>
				...
				<table id="" class="table table-bordered table-striped ">
					...
					?>
						...
						<tr>
							...
							<td>
								<?php 
								$sql2 = "SELECT sum(issue_qty) FROM `inv_issuedetail` where `warehouse_id` = '$warehouse_id' AND `material_id` = '$material_id' AND `issue_date` BETWEEN '$from_date' AND '$to_date'";
